...
- **opencode**：`command` 是数组，可执行文件和参数放一起 `["uvx", "--from", "...", "multimodal-mcp"]`。
- **Claude Code / Desktop / Cursor**：`command` 是字符串（可执行文件），`args` 是数组（参数）。
- **Codex CLI**：TOML 格式，`command` 字符串 + `args` 数组，`env` 是 inline table。

`env` 字段三个客户端都支持，写法见下文。
...
脚本对每个检测到的客户端写入 MCP 配置（含 `env`，如提供了的话）+ 规则文件，幂等可重复跑。不带 `--base-url` 等参数时只写 command/args，你自己补 `env`。

> `--api-key` 会进 shell 历史。介意的话不带这个参数，跑完手动填 `env`。
...
`env` 字段照搬 uvx 模式的。`command` 必须是 venv 里的 python，不是系统的 `python`，否则会缺 `mcp` / `httpx` 等依赖。

### Windsurf / Cline

MCP 配置走各自 UI（Settings > MCP），`command` / `args` / `env` 同上。
